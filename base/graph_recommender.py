...
class GraphRecommender(Recommender):
    ...
    def test(self):
        ...
        for i, user in enumerate(self.data.test_set):
            ...
            if i % 1000 == 0 and self.progress_bar:
    ...
    def evaluate(self, rec_list):
        ...
        self.result.append((self.save_path + '\n'))
        # append logs
        self.result.append('RECALL LOGS:\n')
        self.result.append(self.recall_logs)
        self.result.append('ALIGN LOGS:\n')
        self.result.append(self.align_logs)
        self.result.append('UNIF LOGS:\n')
        self.result.append(self.unif_logs)
        # self.model_log.add('###Evaluation Results###')
        # self.model_log.add(self.result)

        # FileIO.write_file(out_dir, file_name, self.result)
        # print('The result of %s:\n%s' % (self.model_name, ''.join(self.result[:11])))
    ...
    def fast_evaluation(self, epoch):
        ...
        if len(self.bestPerformance) > 0:
            ...
            self.save()
        self.recall_logs += (str(performance['Recall']) + '\n')
        print('-' * 120)
